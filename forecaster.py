...
class ClusterForecaster:
    ...
    def __init__(
        self, train_df, prediction_seqlen, prediction_interval, prediction_horizon, save_path, top_k=5, override=False,
    ):
    ...
    def _train_cluster(self, cluster_counts, cluster, save_path):
        ...
        self.models[cluster] = LSTM(
            horizon=self.prediction_horizon, interval=self.prediction_interval, sequence_length=self.prediction_seqlen,
        )
...
class WorkloadGenerator:
    ...
    def get_workload(self, cluster, cluster_count):
        ...
        templates_with_param_vecs = [
            (template, np.tile(self._preprocessor.sample_params(template, 1)[0], (int(count), 1)),)
            for template, count in zip(templates.index.values, templates.values)
        ]
