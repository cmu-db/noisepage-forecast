...
from constants import DEBUG_POSTGRESQL_CSV_PARSED, CLUSTER_ASSIGNMENT_CSV, CLUSTER_DATA_CSV
...
class Clusterer:
    ...
    def __init__(
        self, dataframe, n_samples=10000, rho=0.7, cluster_interval=pd.Timedelta(seconds=1), online_cluster=False
    ):
        """
        ...
        # Lookback window for online clustering
        w = 40
        self.lookback_window = w * self.interval_delta
    ...
    def _query_df(df, template, timestamps):
        ...
        # The first level can be dropped since query_template == template.
        df = df.query(
            "`query_template` == @template and `log_time_s` in @timestamps").droplevel(0)
    ...
    def _build_neighbors(centers, timestamps, n_neighbors):
        ...
        samples = np.array([Clusterer._query_series(
            centers[cluster], timestamps).values for cluster in clusters])

        if len(samples) == 0:
            neighbors = None
        else:
            samples = samples.reshape(len(clusters), -1)
            normalized_samples = sklearn.preprocessing.normalize(
                samples, copy=False)
    ...
    def _adjust_template(self, template, current_time, old_assignment, timestamps, neighbors):
        ...
        # Only consider a fix-sized lookback window
        start_time = max(self.min_time, end_time - self.lookback_window)
        ...
        if old_assignment is not None:
            ...
            still_belongs = (
                Clusterer._similarity(
                    ...
                    self._query_series(
                        self.centers[old_assignment], timestamps).values,
                )
                > self.rho
            )
            ...
            self._modify_cluster(False, old_assignment,
            ...
            for cluster in self.centers.keys():
                if (
                    self._similarity(
                        self._query_df(self._df, template, timestamps).values,
                        self._query_series(
                            self.centers[cluster], timestamps).values,
                    )
                    > self.rho
                ):
        ...
        if new_assignment is not None:
            ...
            self._modify_cluster(True, new_assignment,
                                 template, start_time, end_time)
    ...
    def _cluster_online(self):
        ...
        # Initialized to be: { {self.min_time, {template1: None, template2: None, ...}} }
        self.assignments = [
            (
                self.min_time,
        ...
        for gap in range(self.n_gaps):
            ...
            # Choices:
            # (1) Consider from self.min_time
            # (2) Consider from next_time - w * self.interval_delta, where w
            # is a configurable window size.
            start_time = max(self.min_time, next_time - self.lookback_window)

            # Total points in range [start_time, next_time]
            total_points = (next_time - start_time) // self.interval_delta + 1

            # Timestamps to consider.
            timestamps = self._sample_timestamps(
                total_points, start_time, next_time, self.n_samples, self.interval_delta)
            ...
            neighbors = self._build_neighbors(
                self.centers, timestamps, n_neighbors=1)
            ...
            neighbors = self._build_neighbors(
                self.centers, timestamps, n_neighbors=2)
                    ...
                    is_similar = (
                        self._similarity(
                            self._query_series(
                                self.centers[cluster], timestamps).values,
                            self._query_series(
                                self.centers[clusters[neighbor]], timestamps).values,
                        )
                        > self.rho
                    )
            ...
            for cluster, df in self.centers.items():
                if df.shape[0] == 0:
                    print(
                        f"WARNING: gap {gap} cluster {cluster} has no items.")
        for template, cluster in self.assignments[-1][1].items():
            print(self._dbgname[template], "->", cluster)
        self.num_clusters = len(self.centers)

        return pd.DataFrame(self.assignments[-1][1].items(), columns=["query_template", "cluster"]).set_index(
            "query_template"
        )
    ...
    def _cluster_offline(self):
        ...
        counts = np.array(
            [
                ...
                self._query_df(self._df, template,
                               timestamps).values.reshape((-1))
                for template in self._get_queries()
            ]
        )

        clustering = DBSCAN(eps=1 - self.rho, metric="cosine",
                            min_samples=1).fit(counts)
        labels = clustering.labels_
        reverse_lookup = {template_id: template_str for template_str,
                          template_id in self._dbgname.items()}
        final_assignments = {
            reverse_lookup[template_id]: cluster_id for template_id, cluster_id in enumerate(labels)}
...
def main():
    # TODO:
    # Concate week data into month data file;
    # See if pandas works. If not, try dask.
    cols = ['log_time', 'query_template']
    df = pd.read_csv(DEBUG_POSTGRESQL_CSV_PARSED, usecols=cols,
                     parse_dates=["log_time"], index_col=False).set_index(["log_time"])
    print(f"Loaded csv: {df.shape}")

    cluster_interval = pd.Timedelta(hours=1)
    df = get_grouped_dataframe_interval(df, cluster_interval)
    print(f"Grouped dataframe interval: {df.shape}")

    print("Clustering query templates.")
    df.index.rename(["query_template", "log_time_s"], inplace=1)
    clusterer = Clusterer(
        df, cluster_interval=cluster_interval, online_cluster=False)
    clusterer.assignment_df.to_csv(CLUSTER_ASSIGNMENT_CSV)
    print("Done!")
